...
def debug_obs(obs, prefix=""):
    """Prints observation type and relevant details."""
    if isinstance(obs, dict):
        print(f"{prefix}: obs type=dict, keys={list(obs.keys())}")
    elif isinstance(obs, np.ndarray):
        print(f"{prefix}: obs type=ndarray, shape={obs.shape}")
    elif isinstance(obs, torch.Tensor):
        print(f"{prefix}: obs type=torch.Tensor, shape={obs.shape}")
    else:
...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    self.safety_config = {
        'collision_cost': 1.0,
        'lane_departure_cost': 0.5,
        'speed_violation_cost': 0.3,
        'proximity_cost': 0.2
    }
    self.cost_history = []
  ...
  def reset(self):
    ...
    self.cumulative_safety_cost = 0.0
    obs = self._get_obs()
    #debug_obs(obs, prefix="CarlaEnv.reset")
    return obs

  
  def _get_cost(self):
    """Calculate comprehensive safety costs for the current step."""
    cost = 0.0

    # Collision cost
    if len(self.collision_hist) > 0:
        collision_intensity = sum(self.collision_hist)
        cost += self.safety_config['collision_cost'] * (1 + collision_intensity)

    # Lane departure cost
    ego_x, ego_y = get_pos(self.ego)
    dis, _ = get_lane_dis(self.waypoints, ego_x, ego_y)
    if abs(dis) > self.out_lane_thres:
        cost += self.safety_config['lane_departure_cost']

    # Speed violation cost
    v = self.ego.get_velocity()
    speed = np.sqrt(v.x**2 + v.y**2)
    if speed > 1.2 * self.desired_speed:
        speed_excess = speed / self.desired_speed
        cost += self.safety_config['speed_violation_cost'] * speed_excess

    # Proximity to other vehicles cost
    if hasattr(self, 'vehicle_front') and self.vehicle_front:
        cost += self.safety_config['proximity_cost']

    # Store and manage cost history
    self.cost_history.append(cost)
    self.cumulative_safety_cost += cost
  ...
  def step(self, action):
    #print(f"DEBUG (carla_env): action received in step function: type={type(action)}, shape={getattr(action, 'shape', 'N/A')}, value={action}")
    # Calculate acceleration and steering
    if self.discrete:
      acc = self.discrete_act[0][action//self.n_steer]
      steer = self.discrete_act[1][action%self.n_steer]
    else:
      #print(f"ENV DEBUG: action type = {type(action)}, value = {action}")
      acc = action[0]
      steer = action[1]
    ...
    safety_cost = self._get_cost()

    # Update info with cost metrics
    info['cost'] = safety_cost
    info['cumulative_safety_cost'] = self.cumulative_safety_cost
    obs = self._get_obs()
    #debug_obs(obs, prefix="CarlaEnv.step")
  ...
  def _init_renderer(self):
    """Initialize the birdeye view renderer with support for headless mode.
    """
    pygame.init()

    # Check if we're in a headless environment
    if 'DISPLAY' in os.environ and os.environ['DISPLAY']:
        # Regular display mode if a display is available
        self.display = pygame.display.set_mode(
            (self.display_size * 3, self.display_size),
            pygame.HWSURFACE | pygame.DOUBLEBUF)
    else:
        # Headless mode using dummy video driver
        os.environ['SDL_VIDEODRIVER'] = 'dummy'
        self.display = pygame.Surface((self.display_size * 3, self.display_size))
        print("Running in headless mode with dummy renderer")
  ...
  def _get_obs(self):
    ...
    for detection in self.lidar_data:
        # Check if we're dealing with the newer LidarDetection format
        if hasattr(detection, 'point'):
            # New API format
            point = detection.point
            point_cloud.append([point.x, point.y, -point.z])
        else:
            # Old API format (deprecated)
            point_cloud.append([detection.x, detection.y, -detection.z])
